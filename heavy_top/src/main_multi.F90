...
program main
   ...
   ! Problem options
   ! Numbers of pendulums
   call aot_get_val(L = conf, key = 'number_heavy_tops', val = prob%number_heavy_tops, ErrCode = iError)
   call error_check(conf, iError, 'number_heavy_tops')
   print *, 'number_heavy_tops = ', prob%number_heavy_tops
   if (prob%number_heavy_tops <= 0) then
      print *, 'FATAL Error: number_heavy_tops must be positive'
      ERROR STOP
   end if

   ! Masses of the pendulums
   if (allocated(ivvError)) deallocate(ivvError)
   call aot_get_val(L = conf, key = 'mass', val = prob%mass, MaxLength = prob%number_heavy_tops, ErrCode = ivvError)
   do i = 1,size(ivvError); call error_check(conf, ivvError(i), 'mass'); end do
   ...
   if (allocated(ivvError)) deallocate(ivvError)
   call aot_get_val(val = prob%inerJ, MaxLength = 3*prob%number_heavy_tops, ErrCode = ivvError, L = conf, key = 'inerJ')
   do i=1,size(ivvError); call error_check(conf, ivvError(i), 'inerJ'); end do
   print *, 'inerJ = [', prob%inerJ, ']'

   ! Initial configuration: x0
   if (allocated(ivvError)) deallocate(ivvError)
   call aot_get_val(val = prob%x0, MaxLength = 3*prob%number_heavy_tops, ErrCode = ivvError, L = conf, key = 'x0')
   do i=1,size(ivvError); call error_check(conf, ivvError(i), 'x0'); end do
   print *, 'x0 = [', prob%x0, ']'

   ! Initial configuration: p0
   if (allocated(ivvError)) deallocate(ivvError)
   call aot_get_val(val = prob%p0, MaxLength = 4*prob%number_heavy_tops, ErrCode = ivvError, L = conf, key = 'p0')
   do i=1,size(ivvError); call error_check(conf, ivvError(i), 'p0'); end do
   print *, 'p0 = [', prob%p0, ']'

   ! Initial configuration: Om0
   if (allocated(ivvError)) deallocate(ivvError)
   call aot_get_val(val = prob%Om0, MaxLength = 3*prob%number_heavy_tops, ErrCode = ivvError, L = conf, key = 'Om0')
   do i=1,size(ivvError); call error_check(conf, ivvError(i), 'Om0'); end do
   print *, 'Om0 = [', prob%Om0, ']'
