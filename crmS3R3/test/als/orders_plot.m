    ...
    if exist('n_figure','var')
        ...
        if side
            if strcmp(sols{i}.problem_name, 'roll-up')
                ...
                title('Roll-up maneuver','FontSize',14,'Interpreter','latex')
                xlabel('Time step size (h)','FontSize',14,'Interpreter','latex')
                ylabel('Global error','FontSize',14,'Interpreter','latex')
                legend('Location','southeast','AutoUpdate','on','Interpreter','latex')
                grid on
                box on
            else
                subplot(1,2,2)
                ...
                set(gca, 'XScale', 'log', 'YScale', 'log')
                axis([10^-4 10^-2 10^-15 10^-4])
                title('Flying spaghetti','FontSize',14,'Interpreter','latex')
                xlabel('Time step size (h)','FontSize',14,'Interpreter','latex')
                ylabel('Global error','FontSize',14,'Interpreter','latex')
                legend('Location','southeast','AutoUpdate','on','Interpreter','latex')
                grid on
        ...
        if side
            ...
            axis([10^-4 10^-2 10^-15 10^-4])
            if length(my_var) == 1
                title(['Order of convergence, ' my_var],'FontSize',14,'Interpreter','latex')
            else
                title('Order of convergence','FontSize',14,'Interpreter','latex')
            end
            xlabel('Time step size (h)','FontSize',14,'Interpreter','latex')
            ylabel('Global error','FontSize',14,'Interpreter','latex')
            legend('Location','best','AutoUpdate','on','Interpreter','latex')
                ...
                switch sols{i}.problem_name
                    ...
                    case 'flying_spaghetti'
                        if exist('pl_fly','var') && ~isempty(pl_fly{sols{i}.order - 1,1})
                            ...
                            pl_fly{sols{i}.order - 1,1} = loglog(sols{i}.h, sols{i}.err.abs.q,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle',the_style{1}, ...
                                'DisplayName',['q, order: ' num2str(sols{i}.order) ', Flying spaghetti']);
                            pl_fly{sols{i}.order - 1,2} = loglog(sols{i}.h, sols{i}.err.abs.v,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{2}, ...
                                'DisplayName',['v, order: ' num2str(sols{i}.order) ', Flying spaghetti']);
                            pl_fly{sols{i}.order - 1,3} = loglog(sols{i}.h, sols{i}.err.abs.l,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{3}, ...
                    ...
                    otherwise
                        if exist('pl_oth','var') && ~isempty(pl_oth{sols{i}.order - 1,1})
                            ...
                            pl_oth{sols{i}.order - 1,1} = loglog(sols{i}.h, sols{i}.err.abs.q,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle',the_style{1}, ...
                                'DisplayName',['q, order: ' num2str(sols{i}.order) ', Other']);
                            pl_oth{sols{i}.order - 1,2} = loglog(sols{i}.h, sols{i}.err.abs.v,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{2}, ...
                                'DisplayName',['v, order: ' num2str(sols{i}.order) ', Other']);
                            pl_oth{sols{i}.order - 1,3} = loglog(sols{i}.h, sols{i}.err.abs.l,'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{3}, ...
            ...
            switch sols{i}.problem_name
                case 'roll-up'
                    if exist('pl_rll','var') && ~isempty(pl_rll{sols{i}.order - 1,1})
                        ...
                        pl_rll{sols{i}.order - 1,1} = loglog(sols{i}.h, sols{i}.err.abs.v,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{2}, ...
                            'DisplayName',['v, order: ' num2str(sols{i}.order) ', Roll-up']);
                        pl_rll{sols{i}.order - 1,2} = loglog(sols{i}.h, sols{i}.err.abs.l,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{3}, ...
                            'DisplayName',['l, order: ' num2str(sols{i}.order) ', Roll-up']);
                    end
                case 'flying_spaghetti'
                    if exist('pl_fly','var') && ~isempty(pl_fly{sols{i}.order - 1,1})
                        ...
                        pl_fly{sols{i}.order - 1,1} = loglog(sols{i}.h, sols{i}.err.abs.v,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{2}, ...
                            'DisplayName',['v, order: ' num2str(sols{i}.order) ', Flying spaghetti']);
                        pl_fly{sols{i}.order - 1,2} = loglog(sols{i}.h, sols{i}.err.abs.l,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{3}, ...
                            'DisplayName',['l, order: ' num2str(sols{i}.order) ', Flying spaghetti']);
                    end
                otherwise
                    if exist('pl_oth','var') && ~isempty(pl_oth{sols{i}.order - 1,1})
                        ...
                        pl_oth{sols{i}.order - 1,1} = loglog(sols{i}.h, sols{i}.err.abs.v,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{2}, ...
                            'DisplayName',['v, order: ' num2str(sols{i}.order) ', Other']);
                        pl_oth{sols{i}.order - 1,2} = loglog(sols{i}.h, sols{i}.err.abs.l,'o-',...
                            'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'LineStyle', the_style{3}, ...
                            'DisplayName',['l, order: ' num2str(sols{i}.order) ', Other']);
                    end
            end
                ...
                switch sols{i}.problem_name
                    case 'roll-up'
                        if exist('pl_rll','var') && ~isempty(pl_rll{sols{i}.order - 1})
                            ...
                            pl_rll{sols{i}.order - 1} = loglog(sols{i}.h,sols{i}.err.abs.(my_var),'o-',...
                    ...
                    case 'flying_spaghetti'
                        if exist('pl_fly','var') && ~isempty(pl_fly{sols{i}.order - 1})
                            ...
                            pl_fly{sols{i}.order - 1} = loglog(sols{i}.h,sols{i}.err.abs.(my_var),'o-',...
                                'Color',colors( - 1 + sols{i}.order,:),'LineWidth',1.5,'DisplayName',['p = ' num2str(sols{i}.order) ]);
                        end
                    otherwise
                        if exist('pl_oth','var') && ~isempty(pl_oth{sols{i}.order - 1})
                            ...
                            pl_oth{sols{i}.order - 1} = loglog(sols{i}.h,sols{i}.err.abs.(my_var),'o-',...
